...
- 选题必须先过“话题度硬门”。话题度不等于热榜排名或播放量：至少要有明确争议或利益冲突、可站队、反转、明星/公众人物/品牌效应、持续二创讨论中的一项。纯政策提醒、企业计划公布、无争议正能量和动物奇闻不得作为高话题度选题；话题度评分低于 20/30 直接淘汰。
...
- 生成至少 5 个候选，再按“话题度、时效、站内热度、共鸣、可核验、可出图”排序。
- 每个候选必须记录 `topic_score`、`discussion_hook`、争议点、可能站队的双方和讨论证据；无法说明“谁会讨论、为什么会站队”的候选直接淘汰。
- 记录候选淘汰原因和最终选择。
...
- 先核验事件，再写标题和正文。
- 状态文件记录 `topic_score` 与 `discussion_hook`，标题和正文首段必须承接该讨论钩子，不能只复述新闻。
- 标题建议格式：`最新事件 + 冲突/问题`。
...
- 向用户展示最终发布摘要：热点及新鲜度、话题度得分与讨论证据、`source.role`、叙事角度、排除的结果事实、独立封面路径与封面事件名、正文四图角色顺序、标题、正文首段、音乐、可见范围，以及用户请求时间、平台最早允许时间和实际回读时间。
